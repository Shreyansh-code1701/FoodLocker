...
<div class="">
    <div class="isotope-grid" id="grid-2">
        ...
        if($str!="")
        {
            ...
            $data = mysqli_query($conn, "select * from item where del=0 and $str order by productid desc");
        }
        else
        {
            $data = mysqli_query($conn, "select * from item where del=0 order by productid desc limit $s,$e");
            if($_REQUEST[stateid]!="" || $_REQUEST[cityid]!="" || $_REQUEST[areaid]!="" || $_REQUEST[keyword]!="" )
            {
                $data=mysqli_query($conn, "select i.* from store s,item i,maincategory m where s.storeid=i.storeid and m.mcatid=i.mcatid and s.stateid=$_REQUEST[stateid] and s.cityid=$_REQUEST[cityid] and s.areaid=$_REQUEST[areaid] and (i.proname like '%$_REQUEST[keyword]%' or m.mcatname like '%$_REQUEST[keyword]%' )  and i.del=0");
            }
            if($_REQUEST[stateid]=="" && $_REQUEST[cityid]=="" && $_REQUEST[areaid]=="" && $_REQUEST[keyword] != "")
            {
                $data=mysqli_query($conn, "select i.* from store s,item i,maincategory m where s.storeid=i.storeid and m.mcatid=i.mcatid  and (i.proname like '%$_REQUEST[keyword]%' or m.mcatname like '%$_REQUEST[keyword]%' )  and i.del=0");
            }
        }
        ...
        {
            $get = mysqli_query($conn, "select m.mcatname,mm.storename from item p,maincategory m,store mm where m.mcatid=p.mcatid and mm.storeid=p.storeid and p.productid=$row[2]");
            ...
            <div class="entry col-xs-12 col-sm-3 col-md-4 filter-food filter-pudding animated zoomInDown">
                <div class="entry-inner">
                    <div class="entry-media">
                        ...
                        <div class="entry-action">
                            <div class="entry-action-inner">
                                ...
                                <?php
                                $i = mysqli_query($conn, "select * from likee where userid like '$_SESSION[user]' and productid = $row[2]");
                                ...
                                $in = mysqli_query($conn, "select count(*) from likee where productid = $row[2]");
                    ...
                    <div class="content-wrapper col-md-12 col-sm-12 col-xs-12 " style="border: 1px dotted #f8a631;">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-6">
                             <?php $rve=  mysqli_query($conn, "select count(reviewid) from reviewproduct where active=1 and productid=$row[2]");
                        $rv=  mysqli_fetch_array($rve);
                        {?>
                    <a href="productdetail.php?proid=<?php echo $row[2]; ?>"><font class="" style="font-size: 13px ;color:#1fb0ab; line-height:1px;"><?php echo $rv[0]; ?> Review</font></a>
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-6">
                                   <?php
                         $av=  mysqli_query($conn, "select avg(rate) from rateproduct where productid=$row[2]");
...
?>
        ...
        <?php
            ...
            $i=  mysqli_query($conn, "select count(productid) from item");
            $ii=  mysqli_fetch_array($i);
