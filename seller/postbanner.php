...
if (isset($_REQUEST[send])) {
        ...
        if ($ex == ".png" || $ex == ".jpg" || $ex == ".jpeg" || $ex == ".PNG" || $ex == ".JPG" || $ex == ".JPEG") 
         {
            if(round($siz = $_FILES[addbanner][size] / 1024)>=0)
            {
                ...
                $get=mysqli_query("select max(adid) from addbanner");
 ...
 $sel=mysqli_query("select * from store where userid like '$_SESSION[user]'");
 $sell=mysqli_fetch_array($sel);
$bdate=date('Y-m-d');
    if($er3!=1 && $er4!=1)
    {
        
          $int=mysqli_query("insert into addbanner values($sell[3],$_REQUEST[timeperiod],0,'$_REQUEST[bannername]','$path1','$bdate','0000-00-00','0000-00-00',0,0)");
    ...
    if($er1!=1 && $er2!=1)
    {
         $int=mysqli_query("update addbanner set mcatid='$_REQUEST[upmaincategory]',productname='$_REQUEST[upproductname]',type='$_REQUEST[upfoodtype]',price=$_REQUEST[upprice],dis='$_REQUEST[updis]','$path3',day=$_REQUEST[upday] where productid=$_SESSION[uid]");
