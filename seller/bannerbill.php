...
<?php
$get = mysqli_query("select s.storename,s.address,a.* from store s,addbanner a where a.storeid = s.storeid and a.adid=(select max(adid) from addbanner where userid like '$_SESSION[user]')");
...
<?php
$g = mysqli_query("select * from timeperiod where timeperiodid=$row[3]");
