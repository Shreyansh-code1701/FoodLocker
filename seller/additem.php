...
if (isset($_REQUEST[send])) {
    if (strlen($_FILES[productpic][name]) > 0) {
        ...
        if ($ex == ".png" || $ex == ".jpg" || $ex == ".jpeg" || $ex == ".PNG" || $ex == ".JPG" || $ex == ".JPEG") {
            if (round($siz = $_FILES[productpic][size] / 1024) >= 0) {
                ...
                $get = mysqli_query("select max(productid) from item");
    ...
    if ($er3 != 1 && $er4 != 1) {
        $int = mysqli_query("insert into item values($_REQUEST[maincategory],$_REQUEST[sellerid],0,'$_REQUEST[productname]','$_REQUEST[foodtype]',$_REQUEST[price],'$_REQUEST[dis]','$path1',0)");
...
if (isset($_REQUEST[upsend])) {
    ...
    $it = mysqli_query("select storeid from store where userid like '$_SESSION[user]' ");
    $item = mysqli_fetch_array($it);

    if ($er1 != 1 && $er2 != 1) {
        $int = mysqli_query("update item set mcatid='$_REQUEST[upmaincategory]',productname='$_REQUEST[upproductname]',type='$_REQUEST[upfoodtype]',price=$_REQUEST[upprice],dis='$_REQUEST[updis]','$path3' where productid=$_SESSION[uid]");
...
<?php
$get = mysqli_query("select * from store where del=0 and userid like '$_SESSION[user]' ");
