...
if ($_REQUEST[konuma] == "misscuisine") {
    ...
    $mc = mysqli_query("select c.*,m.mcatname from cuisine c,maincategory m where m.mcatid = c.mcatid and c.storeid =$_REQUEST[id]");
                        while ($mcc = mysqli_fetch_array($mc)) {
                            ?>
...
if ($_REQUEST[konuma] == "missstoreevent") {
    ...
    $mc = mysqli_query("select e.* from event e,storeevent se,store s where e.eventid=se.eventid and s.storeid=se.storeid and se.storeid=$_REQUEST[id]");
                        while ($mcc = mysqli_fetch_array($mc)) {
                            ?>
...
if ($_REQUEST[tbl] == "city") {
    ...
    $city = mysqli_query("select * from city where del=0 and stateid=$_REQUEST[id]");
...
if ($_REQUEST[tbl] == "area") {
    ...
    $city = mysqli_query("select * from area where del=0 and cityid=$_REQUEST[id]");
    while ($row = mysqli_fetch_array($city)) {
        ?>
        <option value="<?php echo $row[1]; ?>"><?php echo $row[2]; ?></option>
        <?php
...
if ($_REQUEST[kona] == "form") {
 ...
 <div class="form-group">
        ...
        $in=  mysqli_query("select * from item where del=0 and productid=$_REQUEST[id]");
            ...
            <div class="form-group">
                <div class="input-group">
                    ...
                    <select name="upmaincategory" required="" class="form-control">
                        ...
                        <?php
                        $mc = mysqli_query("select * from maincategory where del=0");
                        while ($mcc = mysqli_fetch_array($mc)) {
                            ?>
                            <option value="<?php echo $mcc[0]; ?>"><?php echo $mcc[1]; ?></option>
                            <?php
...
if ($_REQUEST[kona] == "data") {
    if ($_REQUEST[konu] == "item") {
        ...
        if ($_REQUEST[recid] != 0) {
            $up = mysqli_query("update item set del=1 where productid=$_REQUEST[recid]");
        }

        if ($_REQUEST[delid] == 'badhurec') {
            $del = mysqli_query("update item set del=1 where del=0");
        }

        $pp = 8;
        $s=mysqli_query("select storeid from store where userid like '$_SESSION[user]'");
        $ss=mysqli_fetch_array($s);
        $d = mysqli_query("select count(productid) from item where del=0 and storeid=$ss[0]");
        ...
        if ($_REQUEST[ketla] == "all") {
            $data = mysqli_query("select m.mcatname,s.storename,s.userid,i.* from maincategory m,store s,item i where i.del=0 and m.mcatid=i.mcatid and s.storeid=i.storeid and s.userid like '$_SESSION[user]' order by proname limit $st,$pp ");
        } else {
            $data = mysqli_query("select mcatname,storename,i.* from maincategory m,store s,item i where i.del=0 and m.mcatid=i.mcatid and s.storeid=i.storeid  like '$_REQUEST[ketla]%' or s.storename like '$_REQUEST[ketla]%' or i.proname like '$_REQUEST[ketla]%' order by m.mcatname");
        }
...
if ($_REQUEST[kona] == "recdata") {
    if ($_REQUEST[konu] == "item") {
        if($_REQUEST[recid]!=0)
        {
            $up=mysqli_query("update item set del=0 where productid=$_REQUEST[recid]");
        }
        if($_REQUEST[delid]!=0)
        {
            $del=mysqli_query("delete from item where productid=$_REQUEST[delid]");
        }
        if($_REQUEST[badhudel]=="badhu")
        {
            $del=mysqli_query("delete from item where del=1");
        }
        $pp=8;
           $s=mysqli_query("select storeid from store where userid like '$_SESSION[user]'");
        $ss=mysqli_fetch_array($s);
        $d1=  mysqli_query("select count(productid) from item where del=1 and storeid=$ss[0]");
        ...
        <?php
        $c=0;
            $data = mysqli_query("select m.mcatname,s.storename,s.userid,i.* from maincategory m,store s,item i where i.del=1 and m.mcatid=i.mcatid and s.storeid=i.storeid and s.userid like '$_SESSION[user]' order by proname limit $st,$pp ");
...
if($_REQUEST[konu]=='missuserbill')
{
    ...
    {
        $sel=  mysqli_query("select st.statename,c.cityname,a.areaname,s.* from state st,city c,area a,store s where st.stateid=s.stateid and c.cityid=s.cityid and a.areaid=s.areaid and s.userid like '$_SESSION[user]'");
    }
    else
    {
...
<table class="table table-responsive"  >
    ...
    <?php
    $sel = mysqli_query("select * from duration where durationid=$row[4]");
    $gg = mysqli_fetch_array($sel);
    ?>
...
if ($_REQUEST[kona] == "form") {
    if ($_REQUEST[konu] == "addbanner") {
        if ($_REQUEST[sukam] == "insert") {
            ...
            <div class="form-group">
                <div class="input-group">
                    ...
                    <select name="timeperiod" required="" class="form-control">
                        ...
                        <?php
                        $mc = mysqli_query("select * from timeperiod where del=0");
                        while ($mcc = mysqli_fetch_array($mc)) {
                            ?>
                            <option value="<?php echo $mcc[0]; ?>"><?php echo $mcc[1]; ?></option>
                            <?php
            ...
            $_SESSION[uid]=$_REQUEST[id];
        $in=  mysqli_query("select * from addbanner where del=0 and adid=$_REQUEST[id]");
        $du=  mysqli_fetch_array($in);
        ?>
...
if ($_REQUEST[kona] == "data") {
    if ($_REQUEST[konu] == "addbanner") {
        ...
        <table class="table table-responsive table-hover mytable">
            ...
            <th></th>
        <?php
        $bb=mysqli_query("select * from store where userid like '$_SESSION[user]'");
        $bbb=mysqli_fetch_array($bb);
        $c = 0;
        if ($_REQUEST[ketla] == "all") {
         
            $data = mysqli_query("select * from addbanner where del=0 and storeid=$bbb[3]  limit $st,$pp ");
        } else {
...
if ($_REQUEST[kona] == "recdata") {
    if ($_REQUEST[konu] == "addbanner") {
        if($_REQUEST[recid]!=0)
        {
            $up=mysqli_query("update addbanner set del=0 where adid=$_REQUEST[recid]");
        }
        if($_REQUEST[delid]!=0)
        {
            $del=mysqli_query("delete from addbanner where adid=$_REQUEST[delid]");
        }
        if($_REQUEST[badhudel]=="badhu")
        {
            $del=mysqli_query("delete from addbanner where del=1");
        }
        $pp=8;
        $d1=  mysqli_query("select count(adid) from item where del=1");
                            $dd1=  mysqli_fetch_array($d1);
                            $_SESSION[durationreccount]=$dd1[0];
        $page=ceil($_SESSION[durationreccount]/$pp);
        ...
        <?php
        $c=0;
            $data = mysqli_query("select * from addbanner where del=1 order by adbannername limit $st,$pp ");
...
if($_REQUEST[konu]=='missbannerbill')
{
    ...
    if($_REQUEST[kona]=='badha')
    {
        $sel = mysqli_query("select s.storename,s.address,a.* from store s,addbanner a where a.storeid = s.storeid and userid like '$_SESSION[user]'");
    }
    else
    {       
         $sel = mysqli_query("select s.storename,s.address,a.* from store s,addbanner a where a.storeid = s.storeid and  a.storeid =$_REQUEST[kona]");
    }
...
<table class="table table-responsive"  style="background:url(../seller/images/food.png) no-repeat 50% 50%;">
    ...
    <?php
    $sel = mysqli_query("select * from timeperiod where timeperiodid=$row[3]");
    $gg = mysqli_fetch_array($sel);
    ?>
...
if ($_REQUEST[cuisine] == "cuisine") {
    if ($_REQUEST[shu] == "update") {
        ...
        $upcusinie = mysqli_query("insert into cuisine values($_REQUEST[mcatid],$_REQUEST[storeid],0)");
    }
}
...
if ($_REQUEST[cuisine] == "cuisine" && $_REQUEST[shema] == "store" && ($_REQUEST[shu] == "dis" || $_REQUEST[shu] == "update" || $_REQUEST[shu] == "up")) {
    ...
    <form action="" method="post" name="addcuisine" class="form-group storetable">
        <table class="table table-responsive">
    <?php
    $cuisine = mysqli_query("select * from maincategory where mcatid not in (select mcatid from cuisine where storeid = $_REQUEST[storeid]) and del=0");
    while ($fcui = mysqli_fetch_array($cuisine)) {
        ?>
                <tr>
        ...
        if ($_REQUEST[backup] == "backup") {
            if ($_REQUEST[cuisine] == "cuisine" || $_REQUEST[mcat] == "mcat") {
                if ($_REQUEST[shu] == "del") {
                    $upcusinie = mysqli_query("delete from foodlocker.cuisine where storeid=$_REQUEST[storeid] and  mcatid=$_REQUEST[mcatid] ");
                }
            }
        ...
        <form action="" method="post" name="addcuisine" class="form-group storetable">
            <table class="table table-responsive">
        <?php
        $cuisine = mysqli_query("select c.*,m.mcatname from cuisine c,maincategory m where m.mcatid=c.mcatid and c.storeid=$_REQUEST[storeid] and m.del=0");
...
if ($_REQUEST[storeevent] == "storeevent") {
    if ($_REQUEST[shu] == "update") {
        ...
        $upstoreevent = mysqli_query("insert into storeevent values($_REQUEST[storeid],$_REQUEST[eventid],0)");
    }
}
...
if ($_REQUEST[storeevent] == "storeevent" && $_REQUEST[shema] == "store" && ($_REQUEST[shu] == "dis" || $_REQUEST[shu] == "update" || $_REQUEST[shu] == "up")) {
    ...
    <form action="" method="post" name="addstoreevent" class="form-group storetable">
        <table class="table table-responsive">
    <?php
    $even = mysqli_query("select * from event where eventid not in (select eventid from storeevent where storeid = $_REQUEST[storeid]) and del=0");
    while ($fev = mysqli_fetch_array($even)) {
        ?>
                <tr>
        ...
        if ($_REQUEST[backup] == "backup") {
            if ($_REQUEST[storeevent] == "storeevent" || $_REQUEST[event] == "event") {
                if ($_REQUEST[shu] == "del") {
                    $upstoreevent = mysqli_query("delete from foodlocker.storeevent where storeid=$_REQUEST[storeid] and  eventid=$_REQUEST[eventid] ");
                }
            }
        ...
        <form action="" method="post" name="adddevent" class="form-group storetable">
            <table class="table table-responsive">
        <?php
        $storeevent = mysqli_query(" select se.*,e.eventname from storeevent se,event e where e.eventid=se.eventid and se.storeid=$_REQUEST[storeid] and e.del=0");

        while ($fcu = mysqli_fetch_array($storeevent)) {
            ?>
                    <tr>
...
if ($_REQUEST[konu] == "feedbackstore") {
    if ($_REQUEST[sukam] == "delete") {
        $ds = mysqli_query("delete from feedbackstore where feedbackid=$_REQUEST[id]");
    }
    ?>
    <table class="table table-responsiv hale">
        ...
        <th></th>
    <?php
    $se = mysqli_query("select storeid from store where userid like '$_SESSION[user]'");
    while ($see = mysqli_fetch_array($se)) {
        $sc = mysqli_query("select f.*,s.storename from feedbackstore f, store s where s.storeid=f.storeid and s.storeid=$see[0]");
...
if ($_REQUEST[konu] == "contactstore") {
    if ($_REQUEST[sukam] == "delete") {
        $ds = mysqli_query("delete from contactstore where contactid=$_REQUEST[id]");
    }
    ?>
    ...
    $se = mysqli_query("select storeid from store where userid like '$_SESSION[user]'");
    while ($see = mysqli_fetch_array($se)) {
        $sc = mysqli_query("select c.*,s.storename from contactstore c, store s where s.storeid=c.storeid and s.storeid=$see[0]");
        while ($scc = mysqli_fetch_array($sc)) {
            ?>
                <tr>
...
?>
    ...
    $c = 0;
            ...
            {
                $data1=mysqli_query("select s.storename,d.durationtime,d.cost,b.billdate from store s,duration d,packagebill b where s.storeid=b.storeid and d.durationid=s.durationid and d.durationid=b.durationid and s.userid like '$_SESSION[user]' ");
            }
            if($_REQUEST[koni]=="business")
            {
                $data1=mysqli_query("select s.storename,d.durationtime,d.cost,b.billdate from store s,duration d,packagebill b where s.storeid=b.storeid and d.durationid=s.durationid and d.durationid=b.durationid and s.storename like '$_REQUEST[su]%' and s.userid like '$_SESSION[user]' ");
            }
             if($_REQUEST[koni]=="duration")
            {
                $data1=mysqli_query("select s.storename,d.durationtime,d.cost,b.billdate from store s,duration d,packagebill b where s.storeid=b.storeid and d.durationid=s.durationid and d.durationid=b.durationid and d.durationtime like '$_REQUEST[su]%' and s.userid like '$_SESSION[user]' ");
            }
             if($_REQUEST[koni]=="amout")
            {
                $data1=mysqli_query("select s.storename,d.durationtime,d.cost,b.billdate from store s,duration d,packagebill b where s.storeid=b.storeid and d.durationid=s.durationid and d.durationid=b.durationid and d.cost like '$_REQUEST[su]%' and s.userid like '$_SESSION[user]' ");
            }
             if($_REQUEST[koni]=="date")
            {
                $data1=mysqli_query("select s.storename,d.durationtime,d.cost,b.billdate from store s,duration d,packagebill b where s.storeid=b.storeid and d.durationid=s.durationid and d.durationid=b.durationid and b.billdate like '$_REQUEST[su]%' and s.userid like '$_SESSION[user]' ");
            }
...
?>
    ...
    $c = 0;
              ...
              if($_REQUEST[koni]=="badhu")
            {
                    $data1=mysqli_query("select s.userid,s.storename,t.timeperiodname,t.price,b.billdate,a.adbanner from store s,timeperiod t,bannerbill b,addbanner a where s.storeid=b.storeid and t.timeperiodid=b.timeperiodid and a.timeperiodid=t.timeperiodid and s.userid like '$_SESSION[user]' ");
            }
             if($_REQUEST[koni]=="seller")
            {
                    $data1=mysqli_query("select s.userid,s.storename,t.timeperiodname,t.price,b.billdate,a.adbanner from store s,timeperiod t,bannerbill b,addbanner a where s.storeid=b.storeid and t.timeperiodid=b.timeperiodid and a.timeperiodid=t.timeperiodid and s.userid like '$_SESSION[user]' and s.userid like '$_REQUEST[su]%'  ");
            }
            if($_REQUEST[koni]=="business")
            {
                $data1=mysqli_query("select s.userid,s.storename,t.timeperiodname,t.price,b.billdate,a.adbanner from store s,timeperiod t,bannerbill b,addbanner a where s.storeid=b.storeid and t.timeperiodid=b.timeperiodid and a.timeperiodid=t.timeperiodid and s.userid like '$_SESSION[user]' and s.storename like '$_REQUEST[su]%' ");
            }
             if($_REQUEST[koni]=="timeperiod")
            {
             ...
             if($_REQUEST[koni]=="amout")
            {
                $data1=mysqli_query("select s.userid,s.storename,t.timeperiodname,t.price,b.billdate,a.adbanner from store s,timeperiod t,bannerbill b,addbanner a where s.storeid=b.storeid and t.timeperiodid=b.timeperiodid and a.timeperiodid=t.timeperiodid and s.userid like '$_SESSION[user]' and t.price like '$_REQUEST[su]%' ");
            }
...
if ($_REQUEST[kona] == "data") {
    if ($_REQUEST[konu] == "inquiry") {
        ...
        if ($_REQUEST[recid] != 0) {
            $up = mysqli_query("delete from inquiry where inid=$_REQUEST[recid]");
        }

        if ($_REQUEST[delid] == 'badhurec') {
            $del = mysqli_query("delete from inquiry");
        }

        $pp = 8;
        $d = mysqli_query("select count(inid) from inquiry where del=0");
        $dd = mysqli_fetch_array($d);
        $_SESSION[durationcount] = $dd[0];
        $page = ceil($_SESSION[durationcount] / $pp);
        ...
        if ($_REQUEST[ketla] == "all") {
            ...
            $data = mysqli_query("select s.storename,i.proname,i.url,ii.* from store s,item i,inquiry ii where s.storeid=ii.storeid and i.productid=ii.productid and ii.storeid in (select storeid from store where userid like '$_SESSION[user]') limit $st,$pp ");
        } else {
            $data = mysqli_query("select s.storename,i.proname,i.url,ii.* from store s,item i,inquiry ii where s.storeid=ii.storeid and i.productid=ii.productid and ii.storeid in (select storeid from store where userid like 'navjivan') and i.proname like '$_REQUEST[ketla]%'");
        }
    ...
    if($_REQUEST[id]!='0')
    {
        if($_REQUEST[shu]=="store")
        {
            $_SESSION[store]=$_REQUEST[id];
             $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i,user u where u.userid=b.userid and i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and i.storeid=$_REQUEST[id] group by b.billid order by b.billid desc ");
        }
       
        if($_REQUEST[shu]=="billno")
        {
             $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.billid=$_REQUEST[id] and s.userid like '$_SESSION[user]' group by b.billid order by b.billid desc ");
        }
        if($_REQUEST[shu]=="user")
        {
             $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and t.userid like '$_REQUEST[id]' and s.userid like '$_SESSION[user]' group by b.billid order by b.billid desc ");
        }
        if($_REQUEST[shu]=="payname")
        {
             $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.paymethod like '$_REQUEST[id]' and s.userid like '$_SESSION[user]' group by b.billid order by b.billid desc ");
        }
        if($_REQUEST[shu]=="date")
        {
             $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.billdate like '$_REQUEST[id]' and s.userid like '$_SESSION[user]' group by b.billid order by b.billid desc ");
        }
        ...
        if($_REQUEST[lp]=="" && $_REQUEST[hp]!="")
        {
            ...
            $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.finalamount between 0 and $_REQUEST[hp] and s.userid like '$_SESSION[user]' group by b.billid order by b.billid desc ");
        }
        if($_REQUEST[lp]!="" && $_REQUEST[hp]!="")
        {
             
        }  $gh=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.finalamount between $_REQUEST[lp] and $_REQUEST[hp] and s.userid like '$_SESSION[user]' group by b.billid order by b.billid desc ");
     }
    }
    
    while($ghh=  mysqli_fetch_array($gh))
    {
...
<div class="col-md-offset-3 col-md-7"  >
                    ...
                    $dis=0;
                        ...
                        $cs=  mysqli_query("select c.cityname,a.areaname,s.* from city c,area a,shipping s where c.cityid=s.cityid and a.areaid=s.areaid and s.userid like '$ghh[3]'");
...
?>
    ...
    $c = 0;
       ...
       if($_REQUEST[koni]=="store")
            {
                $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and t.productid in ( select productid from item where storeid =$_REQUEST[su]  )");
            }
            if($_REQUEST[koni]=="badhu")
            {
                $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and t.productid in ( select productid from item where storeid =$_REQUEST[su]  )");
            ...
            {
                $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.billid=$_REQUEST[su] and s.userid like '$_SESSION[user]'  ");
            }
             if($_REQUEST[koni]=="paymethod")
            {
                $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.paymethod like '$_REQUEST[su]' and s.userid like '$_SESSION[user]'   ");
            }
            
             if($_REQUEST[koni]=="fdate")
            {
                $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and b.billdate like '$_REQUEST[su]' and s.userid like '$_SESSION[user]'  ");
            }
        ...
        if($_REQUEST[lp]=="" && $_REQUEST[hp]!="")
        {
            ...
            $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and t.grandtotal between 0 and $_REQUEST[hp] and s.userid like '$_SESSION[user]'  ");
        }
        if($_REQUEST[lp]!="" && $_REQUEST[hp]!="")
        {
             
        }  $data1=mysqli_query("select s.userid,s.storeid,t.*,b.* from bill b, transaction t,store s,item i where i.productid = t.productid and s.storeid = i.storeid and b.billid = t.billid and t.grandtotal between $_REQUEST[lp] and $_REQUEST[hp] and s.userid like '$_SESSION[user]'  ");
     }
...
if ($_REQUEST[kona] == "form") {
    if ($_REQUEST[konu] == "offer") {
        if ($_REQUEST[sukam] == "insert") {
            ...
            <div class="form-group">
                <div class="input-group">
                    <select name="mcat" class="form-control">
                        ...
                        <?php
                            $mcat=  mysqli_query("select m.mcatname,m.mcatid,s.storeid,s.userid from maincategory m,store s,cuisine c where m.mcatid=c.mcatid and s.storeid=c.storeid and s.userid like '$_SESSION[user]'");
                        while($mm=  mysqli_fetch_array($mcat))
                        {
                        ?>
            ...
            <div class="form-group">
                <div class="input-group">
                    <select name="store" class="form-control">
                        ...
                        <?php
                            $store=mysqli_query("SELECT storename,storeid from store where userid like '$_SESSION[user]'");
                        while($ss=  mysqli_fetch_array($store))
                        {
                        ?>
            ...
            $_SESSION[uid] = $_REQUEST[id];
            $in = mysqli_query("select * from offer where del=0 and offerid=$_REQUEST[id]");
            $du = mysqli_fetch_array($in);
            ?>
            <div class="form-group">
                <div class="input-group">
...
if ($_REQUEST[kona] == "data") {
    if ($_REQUEST[konu] == "offer") {
        ...
        if ($_REQUEST[recid] != 0) {
            $up = mysqli_query("update offer set del=1 where offerid=$_REQUEST[recid]");
        }

        if ($_REQUEST[delid] == 'badhurec') {
            $del = mysqli_query("update offer set del=1 where del=0");
        }

        $pp = 8;
        $d = mysqli_query("select count(offerid) from offer where del=0");
        $dd = mysqli_fetch_array($d);
        $_SESSION[durationcount] = $dd[0];
        $page = ceil($_SESSION[durationcount] / $pp);
        ...
        $c = 0;
        if ($_REQUEST[ketla] == "all") {
            $data = mysqli_query("select m.mcatname,s.storename,f.* from maincategory m,store s,cuisine c,offer f where f.del=0 and m.mcatid=c.mcatid and s.storeid=c.storeid and m.mcatid=f.mcatid and s.storeid=f.storeid and s.userid like '$_SESSION[user]' order by f.offername limit $st,$pp ");
        } else {
            $data = mysqli_query("select m.mcatname,s.storename,f.* from maincategory m,store s,cuisine c,offer f where f.del=0 and m.mcatid=c.mcatid and s.storeid=c.storeid and m.mcatid=f.mcatid and s.storeid=f.storeid and s.userid like '$_SESSION[user]' and f.offername like '%$_REQUEST[ketla]%'");
        }
        ...
        while ($row = mysqli_fetch_array($data)) {
            ...
            if ($page > 4) {
                ...
                if ($_REQUEST[kona] == "recdata") {
                    if ($_REQUEST[konu] == "offer") {
                        if ($_REQUEST[recid] != 0) {
                            $up = mysqli_query("update offer set del=0 where offerid=$_REQUEST[recid]");
                        }
                        if ($_REQUEST[delid] != 0) {
                            $del = mysqli_query("delete from offer where offerid=$_REQUEST[delid]");
                        }
                        if ($_REQUEST[badhudel] == "badhu") {
                            $del = mysqli_query("delete from offer where del=1");
                        }
                        $pp = 8;
                        $d1 = mysqli_query("select count(offerid) from offer where del=1");
                        $dd1 = mysqli_fetch_array($d1);
                        $_SESSION[durationreccount] = $dd1[0];
                        $page = ceil($_SESSION[durationreccount] / $pp);
        ...
        <?php
        $c = 0;
        $data = mysqli_query("select m.mcatname,s.storename,f.* from maincategory m,store s,cuisine c,offer f where f.del=1 and m.mcatid=c.mcatid and s.storeid=c.storeid and m.mcatid=f.mcatid and s.storeid=f.storeid and s.userid like '$_SESSION[user]' order by f.offername limit $st,$pp ");
...
if ($_REQUEST[kona] == "data") {
    if ($_REQUEST[konu] == "eventprice") {
        ...
        if ($_REQUEST[recid] != 0) {
            $up = mysqli_query("update eventprice set del=1 where eventid=$_REQUEST[recid]");
        }

        if ($_REQUEST[delid] == 'badhurec') {
            $del = mysqli_query("update eventprice set del=1 where del=0");
        }

        $pp = 8;
        $d = mysqli_query("select count(eventid) from eventprice where del=0");
        $dd = mysqli_fetch_array($d);
        $_SESSION[durationcount] = $dd[0];
        $page = ceil($_SESSION[durationcount] / $pp);
        ...
        $c = 0;
        if ($_REQUEST[ketla] == "all") {
            $data = mysqli_query("select ep.eventid,s.storename,e.eventname,ep.price from store s,event e,eventprice ep where s.storeid=ep.storeid and e.eventid=ep.eventid and ep.del=0 and userid like '$_SESSION[user]' order by e.eventname limit $st,$pp ");
        } else {
            $data = mysqli_query("select ep.eventid,s.storename,e.eventname,ep.price from store s,event e,eventprice ep where s.storeid=ep.storeid and e.eventid=ep.eventid and ep.del=0 and userid like '$_SESSION[user]' order by e.eventname and e.eventname like '%$_REQUEST[ketla]%'");
        }
        ...
        while ($row = mysqli_fetch_array($data)) {
            ...
            if ($page > 4) {
                ...
                if ($_REQUEST[kona] == "recdata") {
                    if ($_REQUEST[konu] == "eventprice") {
                        if ($_REQUEST[recid] != 0) {
                            $up = mysqli_query("update eventprice set del=0 where eventid=$_REQUEST[recid]");
                        }
                        if ($_REQUEST[delid] != 0) {
                            $del = mysqli_query("delete from eventprice where eventid=$_REQUEST[delid]");
                        }
                        if ($_REQUEST[badhudel] == "badhu") {
                            $del = mysqli_query("delete from eventprice where del=1");
                        }
                        $pp = 8;
                        $d1 = mysqli_query("select count(eventid) from offer where del=1");
                        $dd1 = mysqli_fetch_array($d1);
                        $_SESSION[durationreccount] = $dd1[0];
                        $page = ceil($_SESSION[durationreccount] / $pp);
        ...
        <?php
        $c = 0;
        $data = mysqli_query("select ep.eventid,s.storename,e.eventname,ep.price from store s,event e,eventprice ep where s.storeid=ep.storeid and e.eventid=ep.eventid and ep.del=1 and userid like '$_SESSION[user]' order by e.eventname limit $st,$pp ");
...
if ($_REQUEST[tbl] == "eventbook") {
    ...
    if ($_REQUEST[work] == "delete") {
        if ($_REQUEST[id] == "all") {
            mysqli_query("delete from eventbook ");
        } else {
            mysqli_query("delete from eventbook where userid like '$_REQUEST[id]'");
        }
    }

    if ($_REQUEST[work] == "confirm") {
        $inb = mysqli_query("select * from eventbook where userid like '$_REQUEST[id]'");
        $inbb = mysqli_fetch_array($inb);
        if ($inbb[7] == 0) {
            $b = mysqli_query("update eventbook set confirm=1 where userid like '$_REQUEST[id]'");
        } else {
            $b = mysqli_query("update eventbook set confirm=0 where userid like '$_REQUEST[id]'");
        }
    }

    if ($_REQUEST[find] == "") {
        $in = mysqli_query("select count(*) from eventbook");
    } else {
        $in = mysqli_query("select count(*) from eventbook where date like '%$_REQUEST[find]%'  ");
    }
    ...
    $c = $st;
    if ($_REQUEST[find] == "") {
        $eb=mysqli_query("select storeid from store where active=1 and userid like '$_SESSION[user]'");
        $ebb=mysqli_fetch_array($eb);
        
        $get = mysqli_query("select s.storename,e.eventname,eb.* from store s,event e,eventbook eb where s.storeid=eb.storeid and e.eventid=eb.eventid and eb.storeid=$ebb[0] order by eventname  limit $st,$pp");
    } else {
        
          $eb=mysqli_query("select storeid from store where active=1 and userid like '$_SESSION[user]");
        $ebb=mysqli_fetch_array($eb);
        $get = mysqli_query("select s.storename,e.eventname,eb.* from store s,event e,eventbook eb where s.storeid=eb.storeid and e.eventid=eb.eventid and eb.storeid=$ebb[0] and eb.date like '%$_REQUEST[find]%' limit $st,$pp");
    }
             ...
             <div class="form-group">
                 <div class="input-group">
                     ...
                     <select name="storeid" class="form-control">
                         ...
                         <?php
                          $b=mysqli_query("select storeid,storename from store where active=1 and userid like '$_SESSION[user]'");
...
if ($_REQUEST[kona] == "data") {
    if ($_REQUEST[konu] == "brochure") {
        ...
        if ($_REQUEST[recid] != 0) {
            $up = mysqli_query("update brochure set del=1 where broid=$_REQUEST[recid]");
        }

        if ($_REQUEST[delid] == 'badhurec') {
            $del = mysqli_query("update brochure set del=1 where del=0");
        }

        $pp = 8;
        $d = mysqli_query("select count(broid) from brochure where del=0");
        $dd = mysqli_fetch_array($d);
        $_SESSION[durationcount] = $dd[0];
        $page = ceil($_SESSION[durationcount] / $pp);
        ...
        $c = 0;
        if ($_REQUEST[ketla] == "all") {
            $data = mysqli_query("select s.storename,b.* from store s,brochure b where s.storeid=b.storeid and b.del=0 and userid like'$_SESSION[user]' limit $st,$pp ");
        } else {
            $data = mysqli_query("select s.storename,b.* from store s,brochure b where s.storeid=b.storeid and b.del=0 and userid like'$_SESSION[user]' and s.storename like '$_REQUEST[ketla]%' or b.broname like '$_REQUEST[ketla]%' order by broname");
        }
...
if ($_REQUEST[kona] == "recdata") {
    if ($_REQUEST[konu] == "brochure") {
        if($_REQUEST[recid]!=0)
        {
            $up=mysqli_query("update brochure set del=0 where broid=$_REQUEST[recid]");
        }
        if($_REQUEST[delid]!=0)
        {
            $del=mysqli_query("delete from brochure where broid=$_REQUEST[delid]");
        }
        if($_REQUEST[badhudel]=="badhu")
        {
            $del=mysqli_query("delete from brochure where del=1");
        }
        $pp=8;
        $d1=  mysqli_query("select count(broid) from brochure where del=1");
                            $dd1=  mysqli_fetch_array($d1);
                            $_SESSION[durationreccount]=$dd1[0];
        $page=ceil($_SESSION[durationreccount]/$pp);
        ...
        <?php
        $c=0;
            $data = mysqli_query("select s.storename,b.* from store s,brochure b where s.storeid=b.storeid and b.del=1 and userid like'$_SESSION[user]' limit $st,$pp ");
...
if($_REQUEST[konu]=="inquiryin")
{
    if($_REQUEST[badhu]=="all")
    {
          ...
          $un=mysqli_query("update inquiry set notification=1");
    ...
    {
    $un=mysqli_query("update inquiry set notification=1 where inid=$_REQUEST[id]");

    $inn=  mysqli_query("select count(*) from inquiry where notification=0");
          $in=  mysqli_fetch_array($inn);
          echo $in[0];
    }
}
...
<h5 class="title">You have 5 Mails </h5>
                    <ul class="dropdown-list normal-list">
                             <?php
                     $pin=  mysqli_query("select u.url,u.proname,r.* from item u,inquiry r where u.productid=r.productid and r.notification=0 order by r.inid desc");
                     while ($in=  mysqli_fetch_array($pin))
                     {
                    ?>
