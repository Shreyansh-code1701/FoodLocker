...
if(isset($_REQUEST[rev]))
{
    ...
    if($r!=1)
    {
        $r=mysqli_query($conn, "insert into review values('$_SESSION[user]','$_REQUEST[pid]',0,'$_REQUEST[review]',0,0)");
...
if(isset($_REQUEST[inq]))
{
    ...
    if($i!=1)
    {
        $i=mysqli_query($conn, "insert into inquery values(0,'$_REQUEST[pid]','$_SESSION[user]','$_REQUEST[contact]','$_REQUEST[email]','$_REQUEST[inquiry]',0)");
    }
    
}
$ak=  mysqli_query($conn, "select * from cart where userid = '$_SESSION[user]'");
$gak=  mysqli_fetch_array($ak);

if (isset($_REQUEST[send])) {
    $date=date('Y-m-d h:i:s');
    $insert = mysqli_query($conn, "insert into reviewstore values('$_SESSION[user]','$_SESSION[id]',0,'$_REQUEST[revmsg]','$date',0,0)");
}

if(isset($_REQUEST[event]))
{
    mysqli_query($conn, "insert into eventbook values('$_SESSION[user]',$_SESSION[id],$_SESSION[eventid],0,$_SESSION[eprice],'$_REQUEST[date]','$_REQUEST[message]',0)");
}
?>
<!DOCTYPE html>
...
?>
    ...
    <div class="mainn" style="padding: 20px;margin-top: -13px;background: white">
      ...
      <div class="content_middle wow bounceInRight" data-wow-delay="0.4s" style="background: #f3f1f2">
        <div class="container" style="margin-left: 50px;">
            ...
            <div class="col-md-12">
                <?php
                    $k=  mysqli_query($conn, "select * from store where storeid='$_SESSION[id]'");
       ...
       <div class="container">
           ...
           <div class="row">
                   ...
                   <div class="main-content">
                       <?php
                       $e=mysqli_query($conn, "select e.* from event e,storeevent s where s.eventid = e.eventid and s.storeid = $_SESSION[id]");
 ...
 <div class="order">
    ...
    <?php
        ...
        $l=  mysqli_query($conn, "select c.*,i.price,i.proname,o.rate from cart c,item i,offer o where i.offerid=o.offerid and c.productid=i.productid and c.userid='$_SESSION[user]'");
        while($ll=  mysqli_fetch_array($l))
        {
            $coun=$coun+$ll[4];
    ...
    <div>
        ...
        <div class="cartad">
            <?php
            $cn=  mysqli_query($conn, "select sum(qty) from cart where userid = '$_SESSION[user]'");
            $acn=  mysqli_fetch_array($cn);
