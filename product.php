<?php
    ...
    if($_REQUEST[cid]!="")
    {
        ...
        $price=mysqli_query($conn, "select price from item where productid=$_SESSION[cid]");
        $pricee=mysqli_fetch_array($price);
        
         $of=  mysqli_query($conn, "select * from offer where del=0");
        ...
        $z=mysqli_query($conn, "select * from cart where productid=$_SESSION[cid] and userid like '$_SESSION[user]'");
        $zz=mysqli_fetch_array($z);
        
        if($zz[1]=="")
        {
             $in=mysqli_query($conn, "insert into cart values('$_SESSION[user]',$_SESSION[cid],0,$_SESSION[qty],$pricee[0],$tot,$dis,$grand)");
        }
        else
        {
            $_SESSION[ch]=1;
                    ...
                    <div class="container" >
                        <div class="row">
                            <div class="col-md-9">
                                ...
                                <?php
$data = mysqli_query($conn, "select * from item where del=0 and storeid=$_SESSION[stid] order by productid desc");
while ($row = mysqli_fetch_array($data)) {
    $get = mysqli_query($conn, "select m.mcatname,mm.storename from item p,maincategory m,store mm where m.mcatid=p.mcatid and mm.storeid=p.storeid and p.productid=$row[2] and p.storeid=$_SESSION[stid] ");
    ...
    if ($row[4] == "veg") {
        ?>
                ...
                <div class="col-md-6 col-sm-6 col-xs-6">
                        <?php $rve=  mysqli_query("select count(reviewid) from reviewproduct where active=1 and productid=$row[2]");
