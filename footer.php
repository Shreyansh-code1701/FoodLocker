    ...
    {
        $se=mysqli_query($conn, "select * from subscriber where email like '$_REQUEST[email]'");
        $ge=mysqli_fetch_array($se);
        
        if($ge[0]=="")
        {
            $in=mysqli_query($conn, "insert into subscriber values(0,'$_REQUEST[email]')");
            $ers=0;
                    ...
                    <div class="row">
                        <div class="col-xs-12 col-sm-3">
                            <div class="ht-widget hw-popular-recipes">
                                ...
                                <?php
                                $rcc=  mysqli_query($conn, "SELECT * FROM item i order by productid desc LIMIT 0,4");
                                while ($rc = mysqli_fetch_array($rcc)) 
                                    {
                                ...
                                <article class="post">
                                    <a href="productdetail.php?proid=<?php echo $rc[2];  ?>">
                                        ...
                                        <div class="content">
                                            ...
                                            <div class="meta-rate" >
                                                 <?php
                         $av=  mysqli_query($conn, "select avg(rate) from rateproduct where productid=$rc[2]");
                    ...
                    ?>
                        ...
                        <div class="col-xs-12 col-sm-3 col-md-3">
                            <div class="ht-widget hw-popular-categories">
                                ...
                                <ul>
                                    <?php $cuu=mysqli_query($conn, "select m.mcatid,m.mcatname,count(m.mcatid) from transaction t,item i,maincategory m where i.productid=t.productid and m.mcatid=i.mcatid and i.del=0 group by m.mcatid order by count(m.mcatid)  desc limit 0,6 ");
 while ($cu=  mysqli_fetch_array($cuu))
 {
                        ...
                        <div class="col-xs-12 col-sm-3">
                            <div class="ht-widget hw-posts">
                                ...
                                <?php
                                $fdd=  mysqli_query($conn, "SELECT * FROM feedback order by feedbackid desc LIMIT 0,5");
                                while ($fd = mysqli_fetch_array($fdd)) 
                                    {
                                ?>
                                <article class="post">
...
<div class=" row " style="background:#333; border-top: 1px solid #F8A631; margin: 0px;  ">
        ...
        <div class="  col-md-4 col-sm-12 col-xs-12 ht-widget hw-subscribe">
            <h3 class="widget-title" style="color:#fff; margin-top: 5%;" >SUBSCRIBE TO OUR EMAIL SUBSCRIBER</h3>
                                <p class="caption">Always up to date about our new food</p>
    ...
    <div class="  col-md-4 payment ht-widget hw-subscribe payment ">
        <h3 class="widget-title" style="color:#fff; margin-top: 1%;" >PAYMENT METHOD</h3>
        <?php
        $sel=  mysqli_query($conn, "select * from paymethod  where del=0");
        while($gg=  mysqli_fetch_array($sel))
        {
        ?>
