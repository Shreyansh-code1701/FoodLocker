...
if($_REQUEST[kona]=="loginpage")
{
     ...
     $lb=mysqli_query($conn, "select * from logblock where userid like '$_REQUEST[user]'");
    $b=mysqli_fetch_array($lb);
    ...
    if($m>=5)
    {
        mysqli_query($conn, "delete from logblock where userid like '$_REQUEST[user]'");
    }
    ...
    $pa=$_REQUEST[user];
    $paa=mysqli_real_escape_string($pa);
    
    $pas=$_REQUEST[password];
    $pass=mysqli_real_escape_string($pas);
    
        
        $a=mysqli_query($conn, "select * from login where userid like '$paa' and password like '$pass'");
        ...
        else
        {
            ...
            $dt=mysqli_query($conn, "select * from login where userid like '$_REQUEST[user]'");
            $dd=mysqli_fetch_array($dt);
            $tm=date('Y-m-d h:i:s');
            if($dd[0]!="")
            {
                 $lb=mysqli_query($conn, "select * from logblock where userid like '$_REQUEST[user]'");
                 ...
                 if($b[2]<3)
                 {
                     if($b[1]=="")
                     {
                           mysqli_query($conn, "insert into logblock values('$_REQUEST[user]',0,1,'$tm')");
                     }
                     else
                     {
                         $try=$b[2]+1;
                         mysqli_query($conn, "update logblock set try=$try,ltime='$tm' where userid like '$_REQUEST[user]'");
                     }
                 }
            ...
            $tp=mysqli_query($conn, "select * from logblock where userid like '$_REQUEST[user]'");
            $gtp=mysqli_fetch_array($tp);
...
{
    $i=mysqli_query($conn, "select * from likee where userid like '$_SESSION[user]' and productid = $_REQUEST[pid]");
    $gi=mysqli_fetch_array($i);
    $f=0;
    if($gi[0]=="")
    {
        $f=1;
        mysqli_query($conn, "insert into likee values('$_SESSION[user]',$_REQUEST[pid],0)");
    }
    else
    {
        $f=2;
        mysqli_query($conn, "delete from likee where likeid = $gi[2]");
    }
    $in=mysqli_query($conn, "select count(*) from likee where productid = $_REQUEST[pid]");
...
$city= mysqli_query($conn, "select * from city where del=0 and stateid=$_REQUEST[id]");
...
$city= mysqli_query($conn, "select * from area where del=0 and cityid=$_REQUEST[id]");
while ($row= mysqli_fetch_array($city))
{
?>
<option value="<?php echo $row[1]; ?>"><?php echo $row[2]; ?></option>
...
if($_REQUEST[konu]=='misscontactstore')
{
    ...
    {
        $consel=  mysqli_query($conn, "select * from store where del=0 and active=1 and storeid=$_REQUEST[kona]");
        $consell=  mysqli_fetch_array($consel);
                        ...
                        <div class=" col-md-offset-1 col-md-4 ht-widget hw-popular-categories">
                            ...
                            <ul style="display: inline;" >
                                ...
                                <li style="list-style: none; margin-top: 7%;">
                                    ...
                                    {
                                        $follow=mysqli_query($conn, "select * from follow where storeid=$_SESSION[storeid] and userid like '$_SESSION[user]'");
                                        $followw=mysqli_fetch_array($follow);
...
if($_REQUEST[shu]=="follow")
{
    if($_REQUEST[val]!=0)
    {
        $del=mysqli_query($conn, "delete from follow where storeid=$_SESSION[storeid] and userid like '$_SESSION[user]' ");
    }
    if($_REQUEST[val]==0)
    {
        $sdate=date('Y-m-d');
        $infollow=mysqli_query($conn, "insert into follow values('$_SESSION[user]',$_SESSION[storeid],0,'$sdate')");
    }
}
?>
...
{
    $k=mysqli_query($conn, "select * from ratestore where userid like '$_SESSION[user]' and storeid=$_SESSION[id] ");
    ...
    if($kk[0]=="")
    {
           if($_REQUEST[val]!=0)
           {
                $rt=  mysqli_query($conn, "insert into ratestore values('$_SESSION[user]',$_SESSION[id],0,$_REQUEST[val])");
           } 
    }
    ...
    else
    {
         if($_REQUEST[val]!=0)
           {
                $rt= mysqli_query($conn, "update ratestore set rate=$_REQUEST[val] where userid like '$_SESSION[user]' and storeid=$_SESSION[id]");
             } 
    }  
    ?>
    ...
    if ($_SESSION[user] == "")
      ...
      ?>
        ...
        $ha=mysqli_query($conn, "select rate  from ratestore where userid like '$_SESSION[user]' and storeid=$_SESSION[id] ");
...
<!------------------------------------------------MANAGE STORE AVG RATE------------------------------------------->    
 ...
 <?php
    ...
    {
                                    $avs=  mysqli_query($conn, "select avg(rate) from ratestore where storeid=$_SESSION[id]");
                                    $aves=  mysqli_fetch_array($avs);
...
if($_REQUEST[kona]=="productrate")
{
     ...
     $k=mysqli_query($conn, "select * from rateproduct where userid like '$_SESSION[user]' and productid=$_SESSION[proid] ");
    $kk=mysqli_fetch_array($k);
    ...
    if($kk[0]=="")
    {
        if($_REQUEST[val]!=0)
        {
            $in=mysqli_query($conn, "insert into rateproduct values('$_SESSION[user]',$_SESSION[proid],0,$_REQUEST[val])");
        }
    }
    else
    {
        if($_REQUEST[val]!=0)
           {
                $rt= mysqli_query($conn, "update rateproduct set rate=$_REQUEST[val] where userid like '$_SESSION[user]' and productid=$_SESSION[proid]");
             } 
    }
...
?>
  ...
  <?php
     ...
     <?php
     $ha = mysqli_query($conn, "select rate from rateproduct where userid like '$_SESSION[user]' and productid=$_SESSION[proid] ");
     $haa = mysqli_fetch_array($ha);
...
?>
    ...
    {
                                    $av=  mysqli_query($conn, "select avg(rate) from rateproduct where productid=$_SESSION[proid]");
                                    $ave=  mysqli_fetch_array($av);
...
<!--------------------------------------------------SHIPPING MISSING FORM--------------------------------------------->
                ...
                if($_REQUEST[kona]=="addform")
                {
                    ...
                    {
                        $_SESSION[alr]=1;
                        $s=mysqli_query($conn, "select * from shipping where shippingid=$_REQUEST[val] ");
                        $ss=mysqli_fetch_array($s);
                    }
                                 $nm=  mysqli_query($conn, "select username from user where userid like '$_SESSION[user]' ");
                                $name=  mysqli_fetch_array($nm);
                                ...
                                <div class="input-group">
                                    <select required="" name="city" class="form-control"  >
                                        <option value="">-Select City-</option>
                                                <?php
                                                $city = mysqli_query($conn, "select * from city where del=0  ");
                                                while ($row = mysqli_fetch_array($city)) {
                                ...
                                <div class="input-group">
                                    <select required="" name="area" class="form-control"  >
                                        <option value="">-Select Area-</option>
                                            <?php
                                            $area = mysqli_query($conn, "select * from area where del=0");
                                            while ($row2 = mysqli_fetch_array($area)) {
...
<!--------------------------------------------------MISSING USER BILL--------------------------------------------->
   ...
   if($_REQUEST[kona]=="missbill")
   {
       if($_REQUEST[su]=="last")
       {
           $ub=mysqli_query($conn, "select * from bill where billid=(select  max(billid) from bill) and userid like '$_SESSION[user]'");
           
       }
       if($_REQUEST[su]=="billno")
       {
           $ub=mysqli_query($conn, "select * from bill where userid like '$_SESSION[user]' and billid=$_REQUEST[val] ");
       }
        if($_REQUEST[su]=="fdate")
       {
           $ub=mysqli_query($conn, "select * from bill where userid like '$_SESSION[user]' and billdate='$_REQUEST[val]' ");
       }
   ?>
...
<div class="col-md-offset-3 col-md-6" style="background: #eee; margin-top: 15px;" >
                    ...
                    while ($ubill = mysqli_fetch_array($ub)) 
                        {
                        
                        $cs=  mysqli_query($conn, "select c.cityname,a.areaname,s.* from city c,area a,shipping s where c.cityid=s.cityid and a.areaid=s.areaid and s.userid like '$_SESSION[user]' ");
                    ...
                    <table class="table table-responsive" >
                        ...
                        <tr>
                           ...
                           $sp=mysqli_query($conn, "select i.proname,s.storename,t.* from item i,transaction t,store s where i.productid = t.productid and s.storeid = i.storeid and t.userid like '$_SESSION[user]' and t.billid=$ubill[1]"); 
...
?>
    ...
    $c = 0;
            ...
            {
                $data1=mysqli_query($conn, "select b.userid,b.billdate,b.paymethod,t.* from bill b,transaction t where b.billid=t.billid and t.userid like '$_SESSION[user]'  ");
            }
            if($_REQUEST[koni]=="billno")
            {
                $data1=mysqli_query($conn, "select b.userid,b.billdate,b.paymethod,t.* from bill b,transaction t where b.billid=t.billid and t.billid like '$_REQUEST[su]%' and t.userid like '$_SESSION[user]' "); 
            }
             if($_REQUEST[koni]=="paymethod")
            {
                $data1=mysqli_query($conn, "select b.userid,b.billdate,b.paymethod,t.* from bill b,transaction t where b.billid=t.billid and b.paymethod like '$_REQUEST[su]%' and t.userid like '$_SESSION[user]'  ");
            }
             if($_REQUEST[koni]=="fdate")
            {
                $data1=mysqli_query($conn, "select b.userid,b.billdate,b.paymethod,t.* from bill b,transaction t where b.billid=t.billid and b.billdate like '$_REQUEST[su]%' and t.userid like '$_SESSION[user]'  ");
            }
             if($_REQUEST[koni]=="low")
            {
                $data1=mysqli_query($conn, "select s.storename,d.durationtime,d.cost,b.billdate from store s,duration d,packagebill b where s.storeid=b.storeid and d.durationid=s.durationid and d.durationid=b.durationid and b.billdate like '$_REQUEST[su]%' and s.userid like '$_SESSION[user]' ");
            }
